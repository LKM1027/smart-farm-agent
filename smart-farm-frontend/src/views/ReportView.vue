      ...
      <div class="grid min-h-[680px] gap-5 lg:grid-cols-[320px_1fr]">
        ...
        <aside class="flex flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          ...
          <div v-if="store.isLoadingReports" class="flex flex-1 flex-col items-center justify-center gap-3 p-8">
            <div class="h-8 w-8 animate-spin rounded-full border-2 border-slate-200 border-t-emerald-600"></div>
          ...
          <!-- 리포트 목록 -->
          <ul v-else class="flex-1 overflow-y-auto divide-y divide-slate-200">
            <li
              ...
              :class="[
                ...
                store.selectedReport?.id === report.id
                  ...
                  : 'border-l-2 border-l-transparent hover:border-l-slate-300 hover:bg-slate-50',
              ]"
            >
              <div class="flex items-start justify-between gap-2">
                <div class="min-w-0">
                  <p class="text-sm font-semibold" :class="store.selectedReport?.id === report.id ? 'text-emerald-700' : 'text-slate-900 group-hover:text-slate-900'">
                    📅 {{ report.report_week }}
                  </p>
        ...
        <main class="flex flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          ...
          >
            ...
            <div>
              <p class="text-lg font-semibold text-slate-900">리포트를 선택하세요</p>
          ...
          <template v-else>
            ...
            <div class="flex items-center justify-between border-b border-slate-200 px-6 py-4">
              <div>
                <p class="text-xs font-semibold uppercase tracking-[0.24em] text-emerald-600">Weekly Report</p>
                <h2 class="mt-1 text-lg font-bold text-slate-900">{{ store.selectedReport.report_week }}</h2>
            ...
            <div class="flex-1 overflow-y-auto p-6 lg:p-8">
              <article
                class="markdown-report prose max-w-none"
